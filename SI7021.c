...
uint8_t user_register_1 = 0x3A;
uint8_t heater_control_register = 0x00000000;

const uint16_t I2C_ADDR = (0x40<<1);           // Si7021 I2C address
const uint8_t  HEATER_CURRENT_OFFSET = 3;      // current value in mA for register value 0
const uint8_t  HEATER_CURRENT_STEP   = 6;      // mA/LSB
...
int8_t r_reg(Si7021_registers_t reg)
{
  ...
  if(HAL_OK != HAL_I2C_Mem_Read(&hi2c1, I2C_ADDR, cmd, 1, data, 1, 200))
    return -1;
  else
    return 0;
}
...
int8_t w_reg(uint8_t value, Si7021_registers_t reg)
{
  ...
  if(HAL_OK != HAL_I2C_Mem_Write(&hi2c1, I2C_ADDR, cmd, 1, &value, 1, 200))
    return -1;
  else
    return 0;
}
...
int8_t r_single_Si7021(float* data, Si7021_measurement_type_t type)
{
  ...
  if(HAL_OK != HAL_I2C_Master_Transmit(&hi2c1, I2C_ADDR, &cmd, 1, 200))
    return -1;
  
  if(HAL_OK != HAL_I2C_Master_Receive(&hi2c1, I2C_ADDR, buffer, 2, 200))
    return -1;
...
int8_t r_both_Si7021(float* humidity, float* temperature)
{
  ...
  if(HAL_I2C_Mem_Read(&hi2c1, I2C_ADDR, cmd, 1, buffer, 3,200) != HAL_OK)
    return 1;
  code = convert_to_uint16(buffer);
